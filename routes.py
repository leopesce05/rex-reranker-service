...
from models import RerankRequest, RerankResponse, DocumentScore, HealthResponse
...
def set_rerank_service(service: RerankService):
...
async def rerank_documents(request: RerankRequest):
    """
    Rerankea documentos basado en una query usando el modelo RexReranker-0.6B.
    
    Args:
        request: Contiene la query, documentos, top_k opcional e instruction opcional
    
    Returns:
        Lista de documentos ordenados por relevancia con sus scores
    """
    ...
    try:
        # Ejecutar reranking
        results, instruction_used = rerank_service.rerank(
            query=request.query,
